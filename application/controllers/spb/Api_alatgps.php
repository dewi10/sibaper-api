...
class Api_alatgps extends Api
{
	public function __construct()
	{
		parent::__construct();
		$this->load->model('Alatgps_m');
	}
	...
	public function getAlatgps()
	{
		...
		if (isset($jsonReq['username']) && isset($jsonReq['password']) && count($jsonReq) == 2) {
			...
			$data = $this->Alatgps_m->getAlatgps()->result_array();
			$res = array();
			if (!empty($data)) {
				foreach ($data as $row) {
	...
	public function getAlatgpsById()
	{
		$jsonReq = json_decode($this->input->raw_input_stream, true);
		if (isset($jsonReq['username']) && isset($jsonReq['password']) && count($jsonReq) == 3) {
			...
			$id = $jsonReq['id'];
			$res = array();
			$data = $this->Api_m->getById($id, 'alat_gps')->row_array();
			if (!empty($data)) {
				$res = $data;
			}
	...
	public function inputAlatgps()
	{
		...
		if (isset($jsonReq['username']) && isset($jsonReq['password']) && count($jsonReq) == 3) {
			...
			// Input alat_gps
			$ins = $jsonReq['data'];
			$this->Api_m->insertData($ins, 'alat_gps');

			$resp['status'] = "OK";
			$resp['description'] = "Alat Gps Saved";
		} else {
			$resp['status'] = "NOK";
			$resp['description'] = "Invalid Format";
		}
	...
	public function updateAlatgps()
	{
		...
		if (isset($jsonReq['username']) && isset($jsonReq['password']) && count($jsonReq) == 4) {
			...
			$this->Api_m->updateData($data, $where, 'alat_gps');

			$resp['status'] = "OK";
			$resp['description'] = "Alat Gps Updated";
		} else {
			$resp['status'] = "NOK";
			$resp['description'] = "Invalid Format";
		}
	...
	public function deleteAlatgps()
	{
		$jsonReq = json_decode($this->input->raw_input_stream, true);
		if (isset($jsonReq['username']) && isset($jsonReq['password']) && count($jsonReq) == 3) {
			$this->haveAccess($jsonReq['username'], $jsonReq['password']);
			$id = $jsonReq['id'];
			$this->Api_m->deleteData($id, 'alat_gps');

			$resp['status'] = "OK";
			$resp['description'] = "Alat Gps Deleted";
		} else {
			$resp['status'] = "NOK";
			$resp['description'] = "Invalid Format";
		}
