...
class Api_akun extends Api
{
	...
	public function getAkun()
	{
		...
		if($this->pagingParamCheck($jsonReq))
		{
			...
			$data = $this->Akun_m->getAkun($jsonReq['limit'], $jsonReq['offset'], $jsonReq['sort_by'], $jsonReq['sort_dir'], $jsonReq['query'], $jsonReq['other'])->result_array();
			$res = array();
			if (!empty($data)) {
				foreach ($data as $row) {
	...
	public function getAkunById()
	{
		...
		if (isset($jsonReq['username']) && isset($jsonReq['password']) && count($jsonReq) == 3) {
			...
			$id = $jsonReq['id'];
			$res = array();
			$data = $this->Api_m->getById($id, 'akun')->row_array();
			if (!empty($data)) {
...
//===========================================================================================
	...
	public function inputAkun()
	{
		...
		if (isset($request['username']) && isset($request['password']) && count($request) == 3) {
			...
			// Input sensor
			$resp['id'] = $this->Api_m->insertData($ins, 'akun');
			
	
			$resp['status'] = "OK";
			$resp['description'] = "Akun Tersimpan";
		} else {
			$resp['status'] = "NOK";
			$resp['description'] = "Invalid Format";
		}
	...
	public function updateAkun()
	{
		...
		if (isset($jsonReq['username']) && isset($jsonReq['password']) && count($jsonReq) == 4) {
			...
			$this->Api_m->updateData($data, $where, 'akun');

			$resp['status'] = "OK";
			$resp['description'] = "Akun Updated";
		} else {
			$resp['status'] = "NOK";
			$resp['description'] = "Invalid Format";
		}
	...
	public function deleteAkun()
	{
		...
		if (isset($jsonReq['username']) && isset($jsonReq['password']) && count($jsonReq) == 3) {
			$this->haveAccess($jsonReq['username'], $jsonReq['password']);
			$id = $jsonReq['id'];
			$this->Api_m->deleteData($id, 'akun');

			$resp['status'] = "OK";
			$resp['description'] = "Akun Deleted";
		} else {
			$resp['status'] = "NOK";
			$resp['description'] = "Invalid Format";
		}
