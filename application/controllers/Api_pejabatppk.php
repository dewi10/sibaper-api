...
class Api_pejabatppk extends Api
{
	public function __construct()
	{
		parent::__construct();
		$this->load->model('PejabatPpk_m');
	}

	
	public function getPejabatPpk()
	{
		...
		if($this->pagingParamCheck($jsonReq))
		{
			...
			$data = $this->PejabatPpk_m->getPejabatPpk($jsonReq['limit'], $jsonReq['offset'], $jsonReq['sort_by'], $jsonReq['sort_dir'], $jsonReq['query'], $jsonReq['other'])->result_array();
			$res = array();
			if (!empty($data)) {
				foreach ($data as $row) {
	...
	public function getPejabatPpkById()
	{
		$jsonReq = json_decode($this->input->raw_input_stream, true);
		if (isset($jsonReq['username']) && isset($jsonReq['password']) && count($jsonReq) == 3) {
			...
			$id = $jsonReq['id'];
			$res = array();
			$data = $this->Api_m->getById($id, 'pejabat_ppk')->row_array();
			if (!empty($data)) {
...
//===========================================================================================
	...
	public function inputPejabatPpk()
	{
		...
		if (isset($request['username']) && isset($request['password']) && count($request) == 3) {
			...
			$params = array('nip_pejabat_ppk','nama_pejabat_ppk','jabatan_pejabat_ppk','status');
			...
			$ins = $data;
			// Input sensor
			$resp['id'] = $this->Api_m->insertData($ins, 'pejabat_ppk');
	
			$resp['status'] = "OK";
			$resp['description'] = "Pejabat Ppk Tersimpan";
		} else {
			$resp['status'] = "NOK";
			$resp['description'] = "Invalid Format";
		}
	...
	public function updatePejabatPpk()
	{
		$jsonReq = json_decode($this->input->raw_input_stream, true);
		if (isset($jsonReq['username']) && isset($jsonReq['password']) && count($jsonReq) == 4) {
			...
			$this->Api_m->updateData($data, $where, 'pejabat_ppk');

			$resp['status'] = "OK";
			$resp['description'] = "Pejabat Ppk Updated";
		} else {
			$resp['status'] = "NOK";
			$resp['description'] = "Invalid Format";
		}
	...
	//-------------------------------------------------------
	public function updatePejabatPpk2()
	{
		if ($this->input->raw_input_stream == "") { // Jika photo tidak di input
			$request = $_POST;
			$data = json_decode($request['data'], true);
	...
	public function deletePejabatPpk()
	{
		$jsonReq = json_decode($this->input->raw_input_stream, true);
		if (isset($jsonReq['username']) && isset($jsonReq['password']) && count($jsonReq) == 3) {
			$this->haveAccess($jsonReq['username'], $jsonReq['password']);
			$id = $jsonReq['id'];
			$this->Api_m->deleteData($id, 'pejabat_ppk');

			$resp['status'] = "OK";
			$resp['description'] = "Pejabat Ppk Deleted";
		} else {
			$resp['status'] = "NOK";
			$resp['description'] = "Invalid Format";
		}
